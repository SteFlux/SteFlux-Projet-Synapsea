...
# --- Début ajout auto-réparation périodique ---
def periodic_auto_repair(interval_minutes=15):
    while True:
        try:
            subprocess.run(["python3", "/root/seia_self_repair.py"], check=True)
            print("✅ Auto-réparation lancée avec succès.")
        except Exception as e:
            print(f"❌ Erreur auto-réparation : {e}")
        time.sleep(interval_minutes * 60)

repair_thread = threading.Thread(target=periodic_auto_repair, daemon=True)
repair_thread.start()
# --- Fin ajout auto-réparation périodique ---
